    ...
    const generateAllMacOsPngImages = (zip, doneCallback) => {
        ...
        const checkIfAllImagesAreGenerated = setInterval(() => {
            if (Object.keys(images).length === sizes.length) {
                ...
                // Just a folder of PNG files
                let zipFolder = zip.folder('Generic Icons');
                ...
                Object.entries(images).forEach(async ([size, { url, dataUrl }]) => {
                    ...
                    zipFolder.file(url, content, {base64: true});
                });
        
                // The same folder formatted for creation of a .icns file using the iconutil command line tool for macOS apps
                zipFolder = zip.folder('App.iconset');

                Object.entries(images).forEach(async ([size, { url, dataUrl }]) => {
                    const substringPoint = dataUrl.indexOf('base64,') + 'base64,'.length;
                    const content = dataUrl.substring(substringPoint);

                    zipFolder.file('icon_' + url, content, {base64: true});

                    // For retina display icons
                    const [x, y] = size.split('x');
                    if (sizes.indexOf((x / 2) + 'x' + (y / 2)) > -1) {
                        zipFolder.file('icon_' + (x / 2) + 'x' + (y / 2) + '@2.png', content, {base64: true});
                    }
                });
        ...
        generateAllMacOsPngImages(zip, () => {
            generateIcoImage(zip, () => {
                generateFullSizePngImage(zip, () => {
                    ...
                    zip.file('README.txt', 'This zip file contains all the icons generated by the Emoji to App Icon Generator by Martin Shaw');
                    zip.file('state.json', JSON.stringify(state, null, undefined));
